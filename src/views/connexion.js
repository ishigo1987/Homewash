exports.create = () =>{
    ...
    const imageView = new ImageView({layoutData:{centerX: 0,width: 250,height:150,top:20},image:"src/img/logoHowash.jpg",scaleMode: "fit", id:"logo"}).appendTo(scrollView);
    ...
    const button = new Button({layoutData:{ top:["prev()", 15],left:"10%",right:"10%"},font: font14px,textColor:"#fff",text:"Connexion",background: themeColor,elevation:0
        ...
        //          });
           if(loginValue === '666666666' && passwordValue === 'Howash'){
               // C'est bon
               localStorage.setItem('storeHowashUserInfos', 'dataUserToStore');
               executeNavigationView.visible = false;
               executeNavigationView.dispose();
               const homeView = require('./home.js');
                     homeView.create();
           }else{
            messageInfo(connexionView,40,'Votre couple login, mot de passe ne correspond pas');
           }
         }
  }).appendTo(scrollView);
  ...
  const inscription = new Button({layoutData:{top:["prev()", 15],left:"10%",right:"10%"},font: font14px,text:"Créer mon compte Howash",textColor:"#212121",
     background: "#eeeeee",
     elevation:0
  }).on("select", () =>{  
